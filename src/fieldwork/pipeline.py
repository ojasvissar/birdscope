...
<title>Fieldwork · Michigan conservation brief</title><style>body{{font:17px/1.65 system-ui,sans-serif;color:#172b32;max-width:880px;margin:48px auto;padding:0 24px}}h1{{font-size:42px;line-height:1.1}}h2{{margin-top:40px}}.tag{{color:#086653;letter-spacing:.12em;font-size:13px}}table{{border-collapse:collapse;width:100%}}td,th{{padding:12px;border-bottom:1px solid #ddd;text-align:left}}aside{{padding:18px;background:#edf5f2}}@media print{{body{{margin:0}}button{{display:none}}}}</style>
<p class="tag">FIELDWORK / SCIENTIFIC BRIEF / OFFICIAL TEACHING DATA</p>
...
<p>Northern, central, and southern bands use 45.5°N and 44°N boundaries. They demonstrate a custom area-of-interest request; they are not ecological or administrative units.</p>
...
<h2>Reproducibility</h2><p>Run {data['run']['id']}; seed {data['run']['seed']}. Input checksums, source fingerprints, versions and configuration are in the downloadable manifest. Results are a portfolio demonstration, not a Cornell-endorsed product.</p>
